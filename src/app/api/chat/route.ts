...
function mergeMelUpdates(currentMel: MelData, updatedInjects: Inject[]): MelData {
  ...
  try {
    ...
    if (extractedJson && Array.isArray(extractedJson)) {
      ...
      if (existingMel && existingMel.injects && existingMel.injects.length > 0) {
        ...
        console.log('Updated MEL to version ' + melData.version + ' - ' + melData.totalInjects + ' injects processed');
